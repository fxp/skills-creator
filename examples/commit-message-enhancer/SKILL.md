...
description: |
  分析 git staged diff 并生成符合 Conventional Commits 规范的提交消息。
  自动推断 type(feat/fix/docs/style/refactor/test/chore)、从路径推断 scope、
  用祈使句写 description。支持中英双语输出。
  触发场景：用户说"帮我写commit"、"生成提交信息"、"commit message"、"git commit"时触发。
  只生成建议，不自动执行提交。
allowed-tools:
  - Bash
  - Read
